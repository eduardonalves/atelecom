...
<td><input type="checkbox" name="chk12" <? if(strstr($USUARIO['colunas_clarotv'],'(data_instalacao)')){?>checked="checked"<? } ?> value="data_instalacao" /> <?= strtoupper($campo->data_instalacao);?></td>
</tr>
<tr align="left">
<td><input type="checkbox" name="chk13" <? if(strstr($USUARIO['colunas_clarotv'],'(endereco)')){?>checked="checked"<? } ?>  value="endereco" /> <?= strtoupper($campo->endereco);?></td>
<td><input type="checkbox" name="chk14" <? if(strstr($USUARIO['colunas_clarotv'],'(tipo_instalacao)')){?>checked="checked"<? } ?> value="tipo_instalacao" /> <?= strtoupper($campo->tipo_instalacao);?></td>
</tr>
<tr align="left">
...
<td><input type="checkbox" name="chk16" <? if(strstr($USUARIO['colunas_clarotv'],'(tecnico)')){?>checked="checked"<? } ?> value="tecnico" /> <?= strtoupper($campo->tecnico);?></td>
</tr>
<tr align="left">
...
<td width="210px" align="center">| Documentos: <input type="radio" name="g" value="1" <? if($_GET['g'] == '1'){?> checked="checked" <? }?> onchange="javascript:document.forms.filtro.submit();" /> Sim <input type="radio" name="g" value="0" <? if($_GET['g'] == '0'){?> checked="checked" <? }?> onchange="javascript:document.forms.filtro.submit();" /> Não</td>
...
<td title="Nome do Monitor" onclick="window.location = '?p=clarotv&m=<?= $_GET['m'];?>&t=<?= $_GET['t'];?>&f=<?= $_GET['f'];?>&s=<?= $_GET['s'];?>&v=<?= $_GET['v'];?>&i=<?= $_GET['i'];?>&b=<?= $_GET['b'];?>&tpv=<?= $_GET['tpv']; ?>&pg=<?php echo ($pg - 1); ?>&di=<?= $_GET['di']; ?>&me=<?= $_GET['me']; ?>&an=<?= $_GET['an']; ?>&ve=<?= $_GET['ve']; ?>&ate=<?= $_GET['ate']; ?>&g=<?= $_GET['g']; ?>&o=<? if($_GET['o'] != 'vendas_clarotv.monitor DESC'){ echo 'vendas_clarotv.monitor DESC'; } else { echo 'vendas_clarotv.monitor ASC'; }?>'">Monitor <? if($_GET['o'] == 'vendas_clarotv.monitor DESC'){ ?><img src="img/seta-d.png" /> <? } else if($_GET['o'] == 'vendas_clarotv.monitor ASC'){ ?> <img src="img/seta-u.png" /> <? } ?></td>
<? } ?>

<? if(strstr($USUARIO['colunas_clarotv'],'(operador)')){?>
<td title="Nome do Operador" onclick="window.location = '?p=clarotv&m=<?= $_GET['m'];?>&t=<?= $_GET['t'];?>&f=<?= $_GET['f'];?>&s=<?= $_GET['s'];?>&v=<?= $_GET['v'];?>&i=<?= $_GET['i'];?>&b=<?= $_GET['b'];?>&tpv=<?= $_GET['tpv']; ?>&tpv=<?= $_GET['tpv']; ?>&pg=<?php echo ($pg - 1); ?>&di=<?= $_GET['di']; ?>&me=<?= $_GET['me']; ?>&an=<?= $_GET['an']; ?>&ve=<?= $_GET['ve']; ?>&ate=<?= $_GET['ate']; ?>&g=<?= $_GET['g']; ?>&o=<? if($_GET['o'] != 'vendas_clarotv.operador DESC'){ echo 'vendas_clarotv.operador DESC'; } else { echo 'vendas_clarotv.operador ASC'; }?>'">Operador <? if($_GET['o'] == 'vendas_clarotv.operador DESC'){ ?><img src="img/seta-d.png" /> <? } else if($_GET['o'] == 'vendas_clarotv.operador ASC'){ ?> <img src="img/seta-u.png" /> <? } ?></td>
<? } ?>
...
<td width="26px">
<? if($USUARIO['inserir_gravacao'] == 1 && $VENDA['gravacao'] == '' && $VENDA['status'] == 'ANEXAR DOCUMENTOS'){?>
<img src="img/icone-anexar.png" title="Anexar Documentos" width="13" height="13" />
<? } ?>

<? if($VENDA['gravacao'] != ''){?>

<a href="upload/documentos/<?php echo $VENDA['gravacao']; ?>" target="_blank"><img src="img/icone-documentos.png" title="Visualizar Anexados" width="18" height="18" /></a>
