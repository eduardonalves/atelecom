...
if($USUARIO['tipo_usuario'] == 'SUPERVISOR'){
	...
	while($row = mysql_fetch_assoc($querymonitores)){
		...
		if($j ==0){
			$loginMONITOR = $loginMONITOR.'  monitor = '.$row['id'].'' ;
...
// Intalações
$conINST = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$idPRODUTO."' && data_instalacao LIKE '%".$ano.$mes.$dia."%' && status = 'CONECTADO' && ($loginMONITOR)");
$totalINST = mysql_num_rows($conINST);

// Instalar
$conINSTA = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$idPRODUTO."' && status = 'INSTALAR' && data LIKE '%".$ano.$mes.$dia."%' && ($loginMONITOR)");
$totalINSTA = mysql_num_rows($conINSTA);

//Restrições
$conRES = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$idPRODUTO."' && status = 'RESTRIÇÃO' && data LIKE '%".$ano.$mes.$dia."%' && ($loginMONITOR)");
...
//Auditoria (Pré-Análise + Análise + Gravar + Aprovado)
$conOUT = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$idPRODUTO."' && (status = 'PRE-ANALISE' || status = 'ANÁLISE' || status = 'ANEXAR DOCUMENTOS' || status = 'APROVADO') && data LIKE '%".$ano.$mes.$dia."%' && ($loginMONITOR)");
...
// Intalações
$conINST = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$idPRODUTO."' && data_instalacao LIKE '%".$ano.$mes.$dia."%' && status = 'CONECTADO' ");
$totalINST = mysql_num_rows($conINST);

// Instalar
$conINSTA = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$idPRODUTO."' && status = 'INSTALAR' && data LIKE '%".$ano.$mes.$dia."%' ");
$totalINSTA = mysql_num_rows($conINSTA);

//Restrições
$conRES = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$idPRODUTO."' && status = 'RESTRIÇÃO' && data LIKE '%".$ano.$mes.$dia."%' ");
$totalRES = mysql_num_rows($conRES);
...
//Auditoria (Pré-Análise + Análise + Gravar + Aprovado)
$conOUT = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$idPRODUTO."' && (status = 'PRE-ANALISE' || status = 'ANÁLISE' || status = 'ANEXAR DOCUMENTOS' || status = 'APROVADO') && data LIKE '%".$ano.$mes.$dia."%' ");
$totalOUT = mysql_num_rows($conOUT);
}
...
document.getElementById('grafico').innerHTML = '<div id="fecharpacotes" onclick="escondergrafico();">VOLTAR</div><iframe frameborder="0" scrolling="no" id="framepacotes" src="grafico/grafico-clarotv-'+gra+'.php?d=<?= $_GET['d'];?>&m=<?= $_GET['m'];?>&a=<?= $_GET['a'];?>"></iframe>';
...
<div id="chart1" class="chart" onMouseOver="showinfo('i1')" onMouseOut="hideinfo('i1')" onClick="mostrargrafico('vendas');">
...
<div id="chart2" class="chart" onMouseOver="showinfo('i2')" onMouseOut="hideinfo('i2')" onClick="mostrargrafico('pacotes');">
...
<div id="chart3" class="chart" onMouseOver="showinfo('i3')" onMouseOut="hideinfo('i3')" onClick="mostrargrafico('pendentes');">
...
<div id="chart6" class="chart" onMouseOver="showinfo('i6')" onMouseOut="hideinfo('i6')" onClick="mostrargrafico('auditoria');">
