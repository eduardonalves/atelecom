...
if($USUARIO['tipo_usuario'] ==  "MONITOR"){
	...
	$conVENDA = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$idPRODUTO."' && status = 'ANEXAR DOCUMENTOS' && (agendGravacao = '0000-00-00 00:00:00' || agendGravacao = '') && monitor ='".$USUARIO['id']."' ORDER BY data DESC, cpf DESC");
	...
	$conVENDA = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$idPRODUTO."' && status = 'ANEXAR DOCUMENTOS' && (agendGravacao = '0000-00-00 00:00:00' || agendGravacao = '') && ($MONITORES1) ORDER BY data DESC, cpf DESC");		
}else{
	$conVENDA = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$idPRODUTO."' && status = 'ANEXAR DOCUMENTOS' && (agendGravacao = '0000-00-00 00:00:00' || agendGravacao = '') ORDER BY data DESC, cpf DESC");
}
...
if($numVENDA == '0'){ echo "Nenhuma Anexação Pendente!";}

else if($numVENDA == '1'){ echo "<b>1</b> Anexação Pendente!";}

else{ echo "<b>".$numVENDA."</b> Anexações Pendentes!"; }
?>
