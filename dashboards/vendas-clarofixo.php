...
if(($USUARIO['tipo_usuario'] == 'MONITOR') || ($USUARIO['tipo_usuario'] == 'SUPERVISOR')){
	...
	$conPEND = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$idPRODUTO."' && (status = 'ENVIAR DOCUMENTOS' || status = 'BOLETO GERADO') && data LIKE '%".$ano.$mes.$dia."%' && ($loginMONITOR)");
	...
	$conOUT = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$idPRODUTO."' && (status = 'PRE-ANALISE' || status = 'ANEXAR DOCUMENTOS' || status = 'DOCUMENTOS ANEXADOS' || status = 'PENDENTE') && data LIKE '%".$ano.$mes.$dia."%' && ($loginMONITOR)");
	...
	// Para finalizar ( Enviar Gravação + Boleto Gerado) 
	$conPEND = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$idPRODUTO."' && (status = 'ANEXAR DOCUMENTOS' || status = 'BOLETO GERADO') && data LIKE '%".$ano.$mes.$dia."%'");
	$totalPEND = mysql_num_rows($conPEND);
	...
	// Tratar Auditoria (Pré-Análise + Gravar + Gravado + Pendente)
	$conOUT = $conexao->query("SELECT * FROM vendas_clarotv WHERE produto = '".$idPRODUTO."' && (status = 'PRE-ANALISE' || status = 'ANEXAR DOCUMENTOS' || status = 'DOCUMENTOS ANEXADOS' || status = 'PENDENTE') && data LIKE '%".$ano.$mes.$dia."%'");
...
document.getElementById('grafico').innerHTML = '<div id="fecharpacotes" onclick="escondergrafico();">VOLTAR</div><iframe frameborder="0" scrolling="no" id="framepacotes" src="grafico/grafico-clarofixo-'+gra+'.php?d=<?= $_GET['d'];?>&m=<?= $_GET['m'];?>&a=<?= $_GET['a'];?>"></iframe>';
...
#x{ position:absolute; width:100%; color:#565656; top:102%; height:20px; font-size:9px; background: #ffffff; /* Old browsers */
