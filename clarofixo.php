...
if($ordem == ''){ $ordem = 'data DESC, id DESC';}
...
<td colspan="2"><div style="width:280px"> | Data Finalizada <br>&nbsp;&nbsp;De: <input type="text" class="datepicker" name="di" id="calendario4" onKeyPress="mascara(this,data)" value="<?= $_GET['di'];?>" maxlength="10" size="6" onchange="javascript:document.forms.filtro.submit();" />
...
<td width="280px" align="left"><span  style="margin-top:-5px;"> | Documentação: </span><input type="radio" name="g" value="1" <? if($_GET['g'] == '1'){?> checked="checked" <? }?> onchange="javascript:document.forms.filtro.submit();" /> Sim <input type="radio" name="g" value="0" <? if($_GET['g'] == '0'){?> checked="checked" <? }?> onchange="javascript:document.forms.filtro.submit();" /> Não</td>

<td> | Tipo Venda: <select name="tpv" onchange="javascript:document.forms.filtro.submit();" style="width:150px;">
				   	<option></option>
...
<img src="img/icone-gravar.png" title="Anexar Documentos" width="13" height="13" onclick="Popup=window.open('upload-gravacao-simples-clarofixo.php?id=<?= $VENDA['id']; ?>','Popup','toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=no,width=470,height=350,left=430,top=100');" />
...
<? if($USUARIO['inserir_gravacao'] == 1 && $VENDA['status'] == 'ANEXAR DOCUMENTOS' && $VENDA['gravacao'] == ''){?>
...
<a href="upload/documentos/<?php echo $VENDA['gravacao']; ?>" target="_blank"><img src="img/icone-documentos.png" title="Visualizar Documentos" width="16" height="16" /></a>
